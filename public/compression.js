...
export function initCompression(t) {
  ...
  function handleFile(file) {
    ...
    if (!file.type.startsWith('image/')) {
      if (errorMsg) { errorMsg.textContent = t.notSupported; errorMsg.style.display = 'block'; }
      return;
    }
    ...
    requestAnimationFrame(function () {
      requestAnimationFrame(function () {
        ...
        if (ratioLabel) ratioLabel.textContent =
          t.ratio + ': 100%  |  ' + t.savings + ': ' + formatBytes(file.size, t.bytes);
        setTimeout(function () { message.textContent = t.compressed; }, 600);
      });
    });
